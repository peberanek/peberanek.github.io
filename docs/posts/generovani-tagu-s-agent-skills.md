...
**_Update_**: [Custom slash command](https://code.claude.com/docs/en/slash-commands#custom-slash-commands) je skvělý způsob jak si ušetřit psaní. Nový příkaz je opět jen soubor v Markdown:
...
**_Update_**: Zjistil jsem, že v toto chvíli je jednodužší generování tagů zahrnout jako součást příkazu. Vyřešilo se tím i povolení skriptu, který je nyní jako `list-tags.py` v top adresáři daného repozitáře:

```markdown title=".claude/commands/finalize.md"
---
