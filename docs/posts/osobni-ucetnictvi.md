...
Následující text je zamyšlením nad posledními 10 lety, kdy jsem si začal vést osobní účetnictví, tj. kompletní přehled majetku a závazků, příjmů a výdajů. Od úplných začátků s tužkou a papírem až po automatizované zpracování většiny transakcí a výpočtů s využitím open-source aplikací. V článku se pokusím popsat své důvody a zkušenosti a odpovědět na otázku, zda a jaký to celé má vůbec smysl.

<!-- more -->

## Proč jsem s osobním účetnictvím začal

V roce 2015 jsem padnul na finanční dno a k tomu skončil na dva měsíce v nemocnici. Celkem drsná životní lekce. Na druhou stranu, na chvíli jsem nemusel řešit účtenky, penále, a především nesplnitelné požadavky lidí v mém okolí, a to mi dalo příležitost se trochu vzpamatovat a zvednout. Shodou okolností se mi v té době dostala do ruky kniha _Bohatý táta, chudý táta_ od Roberta Kiyosakiho a Sharon Lechterové. Třebaže se jedná o poněkud kontroverzního autora (Kiyosaki), některé myšlenky mě motivovaly si udělat v penězích pořádek.
...
Kdo někdy finančně zápasil, asi mi dá za pravdu, že přemýšlet a mluvit v tom stavu o penězích je bolest. Velká bolest. A taky strach, možná i pocit selhání. Člověk by nejraději dělal, že jeho finanční problémy neexistují, vytěsnil je, zapomněl. A tím se vše ještě zhorší. Ono to na první pohled vypadá logicky: protože máme finanční problémy, nechceme o nich mluvit. Já si ale dovolím tvrdit, že je tomu naopak: _protože jsme naučení o naší finanční realitě nemluvit, tabuizovat ji, nepřemýšlet nad ní, tak právě proto máme menší či větší finanční problémy_. A osobní účetnictví je nástroj, jak si toto přemýšlení ulehčit a dát mu systém.
...
!!! Poznámka
    ...
    Pro detailně rozepsané důvody výhod a nevýhod některých aplikací doporučuji [tento přehled](https://beancount.github.io/docs/command_line_accounting_in_context.html#why-not-just-use-a-spreadsheet).
...
V rámci průzkumu jsem objevil [Plain Text Accounting (PTA)](https://plaintextaccounting.org/), což je způsob vedení účetních záznamů v obyčejných textových souborech. Vedle již zmíněné aplikace _ledger_ (nebo [hledger](https://hledger.org/)) tento přístup implementuje i [Beancount](https://github.com/beancount/beancount/), který je napsaný primárně v Pythonu (a částečně v C). Holt k některým věcem jsem potřeboval dospět, resp. být komfortní s prací v textovém editoru a v příkazové řádce.

Není mojí ambicí v tomto článku poskytnout úvod do Beancountu (viz [dokumentace](https://beancount.github.io/docs/index.html)) ani do podvojného účetnictví (viz např. [tento detailní průvodce](https://beancount.github.io/docs/command_line_accounting_in_context.html)). Přesto uvedu pár příkladů a ukázek jako malou ochutnávku.
...
Pro instalaci balíčků je použita aplikace [uv](https://docs.astral.sh/uv/):
...
; Options (1)
option "name_assets" "Majetek"
option "name_liabilities" "Zavazky"
...
6.  Volitelná metadata. Každá direktiva může mít neomezené množství metadat. Více viz [Metadata](https://beancount.github.io/docs/beancount_language_syntax.html#metadata_1).
...
9.  Transakce má nejen popis `"Obed"`, ale i protistranu (_Payee_) `"Restaurace Koruna"`. `*` je příznak (_Flag_). Pomocí příznaku `!` tak lze např. označit transakce, co potřebují další kontrolu.
10. Možnost přidávat volitelná metadata, v tomto případě ID transakce, je velmi praktická. Např. pro pozdější kontrolu při importu, zda už daná transakce v knize a na daném účtu existuje. Tento typ metadat je zpravidla přístupný při volání přes API a transakce tak může zapisovat nějaký automatizační nástroj.
11. Vyžadováno nástrojem Fava. Viz níže.

!!! Poznámka

    Třebaže Beancount plně podporuje diakritiku, v praxi se mi osvědčilo ji nepoužívat. Píšu pak rychleji. Samozřejmě záleži na vkusu, resp. rozložení klávesnice.
...
Aplikace běží jako server, zpravidla na adrese `http://127.0.0.1:5000` (adresa serveru by se měla objevit ve výstupu příkazu).
...
Obrázek výše: Kvůli zachování účetních principů vychází příjmy jako záporné číslo, což je ze začátku dost neintuitivní. Jednou z možných pomůcek je představa, že to jsou peníze, které někomu jinému ubyly.
...
Obrázek výše: Namísto anglických označení Assets, Liabilities a Equity používám raději Majetek, Závazky a Kapitál (automaticky vypočítané čisté jmění, tj. rozdíl mezi majetkem a závazky; princip je stejný jako v případě příjmů - viz výše).
...
Jedním z benefitů Beancountu je, že ho lze volat i jako knihovnu. Např. takto:
...
Provést instrukce v souboru výše lze např. takto:
...
Svého času jsem se rozhodl napsat [Beanclerk](https://github.com/peberanek/beanclerk) (framework pro import transakcí prostřednictvím API). Řadu věcí by určitě šlo udělat jednodušeji a lépe, nicméně funkci spolehlivě plní, takže ho uvádím jako studijní příklad.
...
* vedle vestavěných příkazů jako `bean-check` (validace účetní knihy) jde doinstalovat další pomocné aplikace jako již zmíněná Fava, [beangulp](https://github.com/beancount/beangulp) (framework pro importování transakcí) nebo [pricehist](https://gitlab.com/chrisberkhout/pricehist) (importování směnných kurzů).
* díky podpoře pro prakticky jakékoliv aktivum (cizí měny, kryptoměny, akcie apod.) a možnosti zaznamenávat nákupní ceny (Cost basis) jde použít pro investování a daňové účely,
